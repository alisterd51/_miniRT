...
/*   Updated: 2020/07/14 12:33:41 by anclarma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	moyenne(int param_aa, t_mlx *mlx, int x, int y)
{
    ...
    while (i < param_aa)
    {
        ...
        while(j < param_aa)
        {
            somme = add_vector(somme, int_to_vector(mlx->image[(mlx->height - x - i - 1) * mlx->width + y + j]));
            j++;
        }
        i++;
    }
...
{
	const int	height_native = mlx->obj.resolution->y_size;
	const int	width_native = mlx->obj.resolution->x_size;
	int			*image_final = mlx->image;
	int			param = 16;
	int			x;
	int			y;

	mlx->obj.resolution->y_size /= param;
	mlx->obj.resolution->x_size /= param;
	mlx->image = malloc(mlx->obj.resolution->y_size * mlx->obj.resolution->x_size * sizeof(int));
	scenescan(mlx);
	x = 0;
	while (x < height_native)
	{
		...
		{
			image_final[(height_native - x - 1) * width_native + y] =
				mlx->image[(mlx->obj.resolution->y_size - x / param - 1) * mlx->obj.resolution->x_size + y / param];
			y++;
		}
		x++;
	}
	free(mlx->image);
	mlx->image = image_final;
	mlx->obj.resolution->y_size = height_native;
	mlx->obj.resolution->x_size = width_native;
}
